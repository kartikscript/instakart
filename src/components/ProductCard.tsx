...
const ProductCard = ({data,viewAsCartProd}:{data:Product,viewAsCartProd ?: boolean}) => {
  ...
  return (
    <div className={`min-w-36 gap-2 grid ${viewAsCartProd ? 'grid-cols-[auto_auto_1fr] grid-rows-[auto_auto_auto]' : 'grid-cols-[auto_auto]' } place-items-center justify-items-start text-xs font-semibold p-2 bg-grey-50 dark:bg-dark-grey-50 shadow-md shadow-grey-100 dark:shadow-dark-grey-100/20 rounded-md text-black dark:text-white`}>
        <img
        src={imageSrc}
        alt="product"
     ...
     <p className={`${viewAsCartProd ? 'col-start-2 col-span-1 row-span-1' : 'col-span-2'} font-normal text-grey-150 dark:text-dark-grey-150 text-[10px]`}>{weight*10} g</p>
      ...
      <div className={`col-span-1 ${viewAsCartProd && 'row-span-full col-start-3'} justify-self-end w-fit uppercase text-sm ${quantity?"bg-main text-main-50 dark:bg-dark-main dark:text-dark-main-50 ":'text-main dark:text-dark-main'} border-main dark:border-dark-main border min-w-14 *:w-full py-1 dark:font-black text-center rounded-md transition-all duration-300`}>
        { 
          quantity === 0 ? (
